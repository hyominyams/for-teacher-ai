                ...
                <div className="container mx-auto px-6 max-w-4xl space-y-12">
                    ...
                    <div className="text-center space-y-4">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">
                            업데이트 <span className="text-emerald-500">소식</span>
                        </h1>
                    ...
                    {/* Timeline List */}
                    <div className="relative space-y-8 before:absolute before:inset-0 before:left-8 md:before:left-12 before:h-full before:w-0.5 before:bg-slate-200 before:hidden md:before:block">
                        {UPDATES.map((update, idx) => (
                            <div key={idx} className="relative flex flex-col md:flex-row gap-8 md:gap-12 items-start group">
                                ...
                                <div className="hidden md:flex flex-col items-end w-48 shrink-0 space-y-2 pt-2 relative z-10">
                                    <div className="text-sm font-bold text-slate-400 font-mono tracking-tight">{update.date}</div>
                                    <Badge variant="outline" className="border-slate-200 text-slate-500 font-mono text-[10px]">
                                        {update.version}
                                    </Badge>

                                    {/* Timeline Dot */}
                                    <div className="absolute right-[-29px] top-3 size-3 rounded-full border-2 border-white bg-slate-200 group-hover:bg-emerald-500 group-hover:scale-125 transition-all shadow-sm" />
                                </div>

                                {/* Content Card */}
                                <div className="flex-1 bg-white rounded-[2rem] p-8 border border-slate-100 shadow-xl shadow-slate-200/40 hover:shadow-2xl hover:border-emerald-100 hover:shadow-emerald-100/30 transition-all w-full">
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-3 mb-2 md:hidden">
                                                <Badge variant="secondary" className="bg-slate-100 text-slate-500 text-[10px]">{update.date}</Badge>
                                                <span className="text-xs font-mono text-slate-300">|</span>
                                                <span className="text-xs font-mono font-bold text-slate-400">{update.version}</span>
                                            </div>
                                        ...
                                        <Badge variant="outline" className={`
                                            ...
                                            ${update.type === 'Major' ? 'bg-indigo-50 text-indigo-600' :
                                                update.type === 'Notice' ? 'bg-amber-50 text-amber-600' : 'bg-slate-100 text-slate-500'}
                                        `}>
                                            {update.type}
                                        </Badge>
                                    </div>
